...
export function ReviewDepositModal({
  isOpen,
  onClose,
  onBack,
  amount,
  ...
  note,
  bankId,
}: ReviewDepositModalProps) {
  const [isSuccessModalOpen, setIsSuccessModalOpen] = useState(false)
  const { deposit } = useWalletStore()

  const handleConfirm = async () => {
    try {
      await deposit(amount, "USD", bankId)
    ...
    <>
      <Dialog open={isOpen && !isSuccessModalOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-gray-500">Please review your deposit details before confirming.</p>
      ...
      {isSuccessModalOpen && <DepositSuccessModal isOpen={isSuccessModalOpen} onClose={onClose} amount={amount} />}
    </>
  )
}
